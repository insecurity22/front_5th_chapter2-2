...
import {
  formatPrice,
  formatDiscountRate,
} from "../../refactoring/utils/formatter";
import { useProductDiscount } from "../../refactoring/hooks/useProductDiscount";
...
describe("advanced > ", () => {
  ...
  describe("자유롭게 작성해보세요.", () => {
    test("새로운 유틸 함수를 만든 후에 테스트 코드를 작성해서 실행해보세요", () => {
      ...
      expect(formatPrice(1500)).toBe("1,500원");
      expect(formatPrice(0)).toBe("0원");

      // Test formatDiscountRate function
    ...
    test("새로운 hook 함수를 만든 후에 테스트 코드를 작성해서 실행해보세요.", () => {
      // Test : getMaxDiscount는 가장 높은 할인율을 반환한다
      const { result } = renderHook(() => useProductDiscount());
      ...
      const maxDiscount = result.current.getMaxDiscount(discounts);
      expect(maxDiscount).toBe(0.2);

      // Test : getAppliedDiscount는 할인 조건이 없으면 0을 반환한다
      const item: CartItem = {
        product: {
          id: "p1",
          name: "테스트 상품",
      ...
      const appliedDiscount = result.current.getAppliedDiscount(item);
